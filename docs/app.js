const quotes = [
  "才饮长沙水，又食武昌鱼。万里长江横渡，极目楚天舒。——毛泽东《水调歌头·游泳》",
  "一桥飞架南北，天堑变通途。——毛泽东《水调歌头·游泳》",
  "暮色苍茫看劲松，乱云飞渡仍从容。——毛泽东《七绝·为李进同志题所摄庐山仙人洞照》",
  "冷眼向洋看世界，热风吹雨洒江天。——毛泽东《七律·登庐山》",
  "莫道昆明池水浅，观鱼胜过富春江.——毛泽东《七律·和柳亚子先生》",
  "海内存知己，天涯若比邻。——王勃《送杜少府之任蜀州》",
  "坐地日行八万里，巡天遥看一千河。——毛泽东《七律二首·送瘟神》",
  "不窥牖，见天道。其出弥远，其知弥少。——《老子·道德经》",
  "咫尺之内，便觉万里之遥。——沈括《梦溪笔谈》",
  "结庐在人境，而无车马喧。问君何能尔？心远地自偏。——陶渊明《饮酒·其五》",
  "胸吞百川流，眼界拓五洲。——近代励志联语",
  "乾坤万里眼，时序百年心。——杜甫《中夜》",
  "运筹帷幄之中，决胜千里之外。——司马迁《史记·高祖本纪》",
  "宇宙一何大，万物各自共。——陶渊明《饮酒·其十》",
  "独坐幽篁里，弹琴复长啸。深林人不知，明月来相照。——王维《竹里馆》",
  "身居庙堂之高，心游万仞之外。——古文意蕴",
  "纳须弥于芥子，涉巨海于浮沤。——《维摩诘经》",
  "万物皆备于我，反身而诚，乐莫大焉。——《孟子·尽心上》",
  "此心安处是吾乡。——苏轼《定风波·南海归赠王定国侍人寓娘》",
  "人生如逆旅，我亦是行人。——苏轼《临江仙·送钱穆父》",
  "采菊东篱下，悠然见南山。——陶渊明《饮酒·其五》",
  "大隐隐于市，小隐隐于野。——王康琚《反招隐诗》",
  "步移景换，处处为家。——中国传统民谚",
  "莫思身外无穷事，且尽生前有限杯。——杜甫《绝句漫兴九首》",
  "竹杖芒鞋轻胜马，谁怕？一蓑烟雨任平生。——苏轼《定风波·三月七日》",
  "闲依农圃邻，偶似山林客。——储光羲《田家即事》",
  "虽无丝竹管弦之盛，一觞一咏，亦足以畅叙幽情.——王羲之《兰亭集序》",
  "行到水穷处，坐看云起时。——王维《终南别业》",
  "独往独来，是谓独断。独断如独行，万物莫能阻。——《庄子·外物》",
  "天高任鸟飞，海阔凭鱼跃。——大智禅师《古尊宿语录》",
  "日出而作，日入而息。凿井而饮，耕田而食。帝力于我何有哉！——《击壤歌》",
  "浮云游子意，落日故人情。——李白《送友人》",
  "愿为飞絮无凭藉，来往相思落岸花。——苏曼殊《为调筝人绘像二首》",
  "天下快意之事莫若友，快友好事莫若谈。——蒲松龄《聊斋志异》",
  "五岳寻仙不辞远，一生好入名山游。——李白《庐山谣寄卢侍御虚舟》",
  "闲来垂钓碧溪上，忽复乘舟梦日边。——李白《行路难·其一》",
  "小舟从此逝，江海寄余生。——苏轼《临江仙·夜归临皋》",
  "只在此山中，云深不知处。——贾岛《寻隐者不遇》",
  "问余何意栖碧山，笑而不答心自闲。——李白《山中问答》",
  "其实地上本没有路，走的人多了，也便成了路。——鲁迅《故乡》",
  "世界其实大得很，我们得去看看。——鲁迅《集外集拾遗补编》",
  "愿中国青年都摆脱冷气，只是向上走，不必听自暴自弃者流的话。——鲁迅《随感录四十一》",
  "不仅是奔赴，而且是告别。——鲁迅《野草·过客》",
  "当我沉默着的时候，我觉得充实；我将开口，同时感到空虚。——鲁迅《野草·题辞》",
  "用一根网线，去连接更广阔的世界，去看看别人是怎么生活的。——户晨风",
  "你一个月挣几千块钱，你在焦虑什么？走出去，看看天地有多大。——户晨风",
  "如果在这个地方觉得压抑，那就换个地方，人是活的，不是树。——户晨风",
  "不要总盯着眼前的这一亩三分地，外面的世界精彩得很。——户晨风",
  "我们努力的意义，不就是为了能自由选择生活的方式吗？——户晨风",
  "自信人生二百年，会当水击三千里。——毛泽东《七言诗·残句》",
  "指点江山，激扬文字，粪土当年万户侯。——毛泽东《沁园春·长沙》",
  "雄关漫道真如铁，而今迈步从头越。——毛泽东《忆秦娥·娄山关》",
  "太平世界，环球同此凉热。——毛泽东《念奴娇·昆仑》",
  "装点此关山，今朝更好看。——毛泽东《菩萨蛮·大柏地》",
  "独立寒秋，湘江北去，橘子洲头。——毛泽东《沁园春·长沙》",
  "喜看稻菽千重浪，遍地英雄下夕烟。——毛泽东《七律·到韶山》",
  "江山如此多娇，引无数英雄竞折腰。——毛泽东《沁园春·雪》",
  "牢骚太盛防肠断，风物长宜放眼量。——毛泽东《七律·和柳亚子先生》",
  "只争朝夕，不负韶华。——近代名言语录",
  "做一只自由的飞鸟，而不是被社会时钟困住的囚徒。——当代数字游民宣言",
  "键盘是我的武器，屏幕是我的窗口，世界是我的办公室。——当代远程工作者",
  "摆脱格子间的束缚，我们在云端重聚。——《数字游民中国指南》",
  "地理套利不是逃避，而是用更聪明的路径去生活。——网络极客语录",
  "在生活成本低的地方，赚取高购买力的货币。——独立开发者社区",
  "你的生活不该被一张打卡机定义。——远程办公倡导者",
  "世界是一本书，不旅行的人只读了其中一页。——圣·奥古斯丁",
  "并非所有流浪者都迷失了方向。——J.R.R. 托尔金《魔戒》",
  "自由不仅意味着摆脱枷锁，更意味着以一种尊重和促进他人自由的方式生活。——纳尔逊·曼德拉",
  "选择你喜欢的工作，你这辈子就一天也不用工作。——国外常引用的译出版表述",
  "生命是一场冒险，或者什么都不是。——海伦·凯勒",
  "如果你的工作不需要你待在特定地方，那你为什么还要待在那里？——蒂姆·费里斯《每周工作4小时》",
  "朝九晚五是二十世纪的工业遗留物，而我们生活在二十一世纪。——杰森·弗里德《远程工作》",
  "游民是一种心态，而不是一个目的地。——罗尔夫·波茨《流浪学》",
  "财富不是拥有多少物质，而是拥有多少选择权。——纳西姆·塔勒布《反脆弱》",
  "你必须在创造自由的同时，学会与孤独相处。——让-保罗·萨特",
  "我们不仅在寻找工作，我们更在寻找一种更有意义的生存方式。——皮特·莱维斯《数字游民》",
  "真正的旅行不是寻找新的景观，而是拥有新的眼光。——马塞尔·普鲁斯特",
  "顺应你的天赋，在云端寻找你的同类。——纳瓦尔·拉维康特《硅谷投资人的思想夜札》",
  "如果你认为冒险是危险的，试试常规吧，它是致命的。——保罗·科埃略",
  "人是生而自由的，但却无往不在枷锁之中。——卢梭《社会契约论》",
  "生活在别处。——米兰·昆德拉",
  "重要的一点是，要敢于在任何时候放弃我们现在的身份，以便成为我们可能成为的人。——查尔斯·杜希格",
  "科技应该用来解放人类，而不是制造更高级的数字工厂。——凯文·凯利",
  "真正的自由是能对你不喜欢的事情说‘不’。——尼采",
  "去过一种你想在临终前回忆的生活。——斯蒂夫·乔布斯",
  "不要去追随前人的足迹，去走没有路的地方，并留下脚印。——拉尔夫·沃尔多·爱默生",
  "千里之行，始于足下。——老子",
  "我宁愿因尝试而失败，也不愿因不尝试而抱憾终生。——巴塔哥尼亚创始人 伊冯·乔伊纳德",
  "未来已经来临，只是分布不均。——威廉·吉布森",
  "我们用代码构建世界，用脚步丈量地球。——开源社区格言",
  "网络消除了距离，而游民消除了职场的围墙。——马特·穆伦维格",
  "只要有一个背包，一个笔记本，一处稳定的Wi-Fi，天地皆可为家。——数字游民部落",
  "不要因为害怕打破常规而选择平庸。——埃隆·马斯克",
  "工作是一件你做的事，而不是一个你去的地方。——远程协作共识",
  "当生活在不同的文化中时，你会发现人性最本质的共通点。——克里斯·吉尔博《100美元起家》",
  "把生产力留给云端，把生活留给自己。——`digital-nomad-cn` 项目主旨",
  "人生最自由的时刻，是发现自己可以随时打包离开的时刻。——流浪极客随笔",
  "在数字的世界里连接，在真实的世界里栖息。——《连线》杂志",
  "地球很大，网速很快，人生很短，不如出发。——数字游民社群结语",
  "我们要运用脑髓，睁开眼来，自己拿来！——鲁迅《拿来主义》",
  "什么是路？就是从没路的地方践踏出来的，从只有荆棘的地方开辟出来的。——鲁迅《生命的路》",
  "世上本没有固定的职业，走得远了，便成了自己的产业。——游民极客随笔",
  "与其在同一个地方内卷，不如去全世界寻找你的生存套利点。——独立黑客周刊",
  "生活不该是死守着一个坐标，而应该是一场全球范围内的流浪和创造。——《游民地球》",
  "你必须要走出你出生的那个村子，去看看太阳在别的地方是怎么升起的。——户晨风",
  "把肉体放在阳光充足的地方，把服务器留在效率最高的地方。——现代地理套利学",
  "自由的代价是绝对的自律，而自由的奖赏是整个世界。——数字游民共识"
];

const quoteTarget = document.querySelector('#random-quote');
if (quoteTarget) quoteTarget.textContent = quotes[Math.floor(Math.random() * quotes.length)];

const categoryIds = ['banking', 'phone', 'securities'];
document.querySelectorAll('.tool-category').forEach((category, index) => {
  if (!category.id) category.id = categoryIds[index] ?? category.id;
});

const menuButton = document.querySelector('[data-menu-toggle]');
const menu = document.querySelector('[data-menu]');
menuButton?.addEventListener('click', () => {
  const isOpen = menu.classList.toggle('is-open');
  menuButton.setAttribute('aria-expanded', String(isOpen));
});
const notice = document.querySelector('[data-notice]');
if (localStorage.getItem('nomad-notice-dismissed') === 'true') notice?.remove();
document.querySelector('[data-notice-close]')?.addEventListener('click', () => {
  localStorage.setItem('nomad-notice-dismissed', 'true');
  notice?.remove();
});

if (window.gsap && window.ScrollTrigger) {
gsap.registerPlugin(ScrollTrigger);

const mm = gsap.matchMedia();

mm.add({
  reduceMotion: '(prefers-reduced-motion: reduce)',
  desktop: '(min-width: 701px)',
}, ({ conditions }) => {
  const { reduceMotion, desktop } = conditions;

  if (reduceMotion) return;

  gsap.timeline({ defaults: { duration: 0.72, ease: 'power3.out' } })
    .from('.hero .eyebrow', { autoAlpha: 0, y: 18 })
    .from('.hero h1', { autoAlpha: 0, y: 34 }, '-=0.42')
    .from('.hero-copy', { autoAlpha: 0, y: 22 }, '-=0.42')
    .from('.hero-quote-container', { autoAlpha: 0, y: 16 }, '-=0.36');

  gsap.from('.section-heading', {
    autoAlpha: 0,
    y: 30,
    duration: 0.7,
    ease: 'power3.out',
    scrollTrigger: {
      trigger: '#modules',
      start: 'top 78%',
      toggleActions: 'play none none reverse',
    },
  });

  ScrollTrigger.batch('.module-card', {
    start: 'top 82%',
    once: true,
    interval: 0.12,
    batchMax: 3,
    onEnter: (cards) => gsap.from(cards, {
      autoAlpha: 0,
      y: 44,
      scale: 0.97,
      duration: 0.7,
      ease: 'power3.out',
      stagger: 0.12,
      overwrite: 'auto',
    }),
  });

  gsap.utils.toArray('.module-visual').forEach((visual) => {
    gsap.fromTo(visual, { yPercent: desktop ? -5 : 0 }, {
      yPercent: desktop ? 5 : 0,
      ease: 'none',
      scrollTrigger: {
        trigger: visual.closest('.module-card'),
        start: 'top bottom',
        end: 'bottom top',
        scrub: 0.6,
      },
    });
  });

  gsap.from('.principle p', {
    autoAlpha: 0,
    y: 24,
    duration: 0.65,
    ease: 'power3.out',
    stagger: 0.14,
    scrollTrigger: {
      trigger: '.principle',
      start: 'top 80%',
      toggleActions: 'play none none reverse',
    },
  });

  if (desktop && window.matchMedia('(pointer: fine)').matches) {
    document.querySelectorAll('[data-tilt]').forEach((card) => {
      card.addEventListener('pointermove', (event) => {
        const bounds = card.getBoundingClientRect();
        const x = (event.clientX - bounds.left) / bounds.width - 0.5;
        const y = (event.clientY - bounds.top) / bounds.height - 0.5;
        gsap.to(card, {
          rotationX: -y * 4,
          rotationY: x * 4,
          transformPerspective: 900,
          duration: 0.25,
          ease: 'power2.out',
          overwrite: 'auto',
        });
      });
      card.addEventListener('pointerleave', () => {
        gsap.to(card, { rotationX: 0, rotationY: 0, duration: 0.45, ease: 'power3.out' });
      });
    });
  }

  document.fonts?.ready?.then(() => ScrollTrigger.refresh());
});
}
