...
// Load GeoJSON
const geojson = JSON.parse(fs.readFileSync('./website/world-geo-raw.json', 'utf8'));
...
function processPolygon(rings, radius) {
    const outerRing = simplifyRing(rings[0], 3);
...
for (const nameEn of targetCountries) {
    ...
    // Special handling for Singapore, Hong Kong, Macao
    if (nameEn === 'Singapore') {
        const coords = [[[103.6,1.15],[104.1,1.15],[104.1,1.5],[103.6,1.5],[103.6,1.15]]];
        const data = processPolygon(coords, 1.01);
    ...
    if (geom.type === 'Polygon') {
        const data = processPolygon(geom.coordinates, 1.01);
        allTriangles = data.triangles;
        allLines = data.lines;
    } else if (geom.type === 'MultiPolygon') {
        for (const polygon of geom.coordinates) {
            const data = processPolygon(polygon, 1.01);
...
fs.writeFileSync('./website/world-geo.json', JSON.stringify(result));
console.log('Done! Output:', Object.keys(result).length, 'countries');
console.log('File size:', fs.statSync('./website/world-geo.json').size, 'bytes');
